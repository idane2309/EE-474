 ...
 * 1937153 & 2068579
 * 11/17/2022 
 * Initialization File and set up
 */
...
//LED AND SWITCHES PORTS AND PINS INITIALIZATION
void LED_Init(void) {
...
void UART_initialize(void) {
  RCGCUART |= 0x1;  //Enable UART0
  RCGCGPIO |= 0x1;   //Enable Port A
  Delay(1);
  GPIODEN_A = 0x3;  //SET PINS PA0/PA1 to Digital Output
  GPIOAFSEL_A = 0x3; //PA0/PA1 ALTERNATE FUNCTION
  GPIOPCTL_A = 0x11;  //SET PORT MUX CONTROL FOR BOTH PINS PA1/PA0 FOR UART0
  Delay(1);
  UARTCTL = 0x0;   //DISABLE UART0 FOR CONFIGURATION
  UARTIBRD = 390;   //UART BAUD RATE = (F / 16 x BAUD DIVISOR) --> 9600 = (60 / 16 x BAUD DIVISOR) --> BAUD DIVISOR = 104.667 --> IBRD = 390
  UARTFBRG = 40;    //FRACTION PART OF BAUD GENERATOR:(0.625 x 64) + 0.5 = 40
  UARTCC = 0x0;    //SELECT THE SYSTEM CLOCK FOR UART
  UARTLCRH = 0x60; //SET WORD LENGTH 8 BITS. IF WANT TO ENABLE FIFO THEN SET TO 0x70 to expand to 16 byte buffer,=.
  UARTCTL = 0x301; //ENABLE TXE, RXE AND UARTEN IN CTL 
}
